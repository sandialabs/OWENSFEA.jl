...
function runme(islinear)
    ...
    for (ii,load) in enumerate(P)
        ...
        for (i,time) in enumerate(delta_t:delta_t:finalt)
            ...
            while uNorm > TOL || aziNorm > TOL #TODO: add TOL to feamodel
                ...
                # Juno.@enter GyricFEA.structuralDynamicsTransient(feamodel,mesh,el,dispData,Omega_j,OmegaDot_j,time,delta_t,elStorage,Fexternal,Fdof,CN2H,rbData;getLinearizedMatrices=false)
    ...
    return meshx, meshz,pointvec, deformedxyz,dispAnalytical,Ux_beam,uHist,P
end

meshx, meshz, pointvec, deformedxyz,dispAnalytical,Ux_beam, uHist,P = runme(true)
_, _, _, deformedxyz_nl,_,Ux_beam_nl, uHist_nl,_ = runme(false) #nonlinear
...
#     PyPlot.plot(meshx,meshz,"k-",label="Undeformed")
#     PyPlot.plot(pointvec[:,3]+deformedxyz[:,3,iload]*deformFact,pointvec[:,1]+deformedxyz[:,1,iload]*deformFact.+0.005,"-",color=plot_cycle[2],label="GXBeam Linear")
#     PyPlot.plot((meshx+Ux*deformFact),(meshz+Uz*deformFact),"-",color=plot_cycle[1],label="OWENS Linear")
#     PyPlot.plot(pointvec[:,3]+deformedxyz_nl[:,3,iload]*deformFact,pointvec[:,1]+deformedxyz_nl[:,1,iload]*deformFact,"--",color=plot_cycle[2],label="GXBeam Nonlinear")
#     PyPlot.plot((meshx+Ux_nl*deformFact),(meshz+Uz_nl*deformFact),"--",color=plot_cycle[1],label="OWENS Nonlinear")
#     PyPlot.legend()
...
#
# PyPlot.figure()
# PyPlot.plot(P,dispAnalytical,"k",label="Anaytical (Linear)")
# PyPlot.plot(P,deformedxyz[end,3,:].+0.001,color=plot_cycle[2],label="GXBeam Linear")
# PyPlot.plot(P,Ux_beam,color=plot_cycle[1],label="OWENS Linear")
...
for iload = 1:length(P)
    atol = dispAnalytical[iload]*0.005 # 0.5%
    @test isapprox(Ux_beam[iload],dispAnalytical[iload];atol)
    @test isapprox(Ux_beam[iload],deformedxyz[end,3,iload];atol)
    # println((Ux_beam[iload]-dispAnalytical[iload])/dispAnalytical[iload]* -100)
    # println((deformedxyz[end,3,iload]-dispAnalytical[iload])/dispAnalytical[iload]* 100)
end

for iload = 1:4 #level 4 produces about 20% deflection
    atol = deformedxyz_nl[end,3,iload]*0.05 # 5%
    @test isapprox(Ux_beam_nl[iload],deformedxyz_nl[end,3,iload];atol)
    # println((deformedxyz_nl[end,3,iload]-Ux_beam_nl[iload])/deformedxyz_nl[end,3,iload]* 100)
end
