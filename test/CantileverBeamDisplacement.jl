...
for imesh = 1:length(mesh.z)-1
    ...
    sectionPropsArray[imesh] = GyricFEA.SectionPropsArray(ac,twist_d,rhoA,EIyy,EIzz,GJ,EA,rhoIyy,rhoIzz,rhoJ,zcm,ycm,a,EIyz,alpha1,alpha2,alpha3,alpha4,alpha5,alpha6,rhoIyz,b,a0,aeroCenterOffset)
end
...
for (iload,load) in enumerate(P)
    ...
    for (i,time) in enumerate(delta_t:delta_t:finalt)
        ...
        while uNorm > TOL || aziNorm > TOL #TODO: add TOL to feamodel
            ...
            if steady
                dispOut,elStrain,staticAnalysisSuccessful,FReaction_j = GyricFEA.staticAnalysis(feamodel,
...
#     PyPlot.plot(pointvec[:,3]+deformedxyz[:,3,iload]*deformFact,pointvec[:,1]+deformedxyz[:,1,iload]*deformFact.+0.005,"-",color=plot_cycle[2],label="GXBeam")
#     PyPlot.plot((meshx+Ux*deformFact),(meshz+Uz*deformFact),"-",color=plot_cycle[1],label="OWENS")
